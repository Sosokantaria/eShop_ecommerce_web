...
export default function SearchedProducts() {
  ...
  async function SearchedProducts() {
    ...
    try {
      ...
      const resp = await axios.get(
        `http://localhost:3001/products?search=${params.value}&skip=${skipedProducts}&take=${10}`
      );
       setTotalItems((await axios.get( `http://localhost:3001/products?search=${params.value}`)).data.length)
  ...
  return (
    <div
      className="flex flex-col items-center flex-wrap m-auto w-full p-6 justify-center gap-3"
